module ReorderTagExtensions
  ...
  tag "next" do |tag|
    ...
    next_page = Page.find_by_parent_id(tag.locals.page.parent_id, :conditions => ["virtual = false and status_id = 100 and ? > ?", by, tag.locals.page.send(by)], :order => "#{by} ASC", :limit => 1)
    if next_page
  ...
  tag "previous" do |tag|
    by = tag.attr['by'] || "position"
    previous_page = Page.find_by_parent_id(tag.locals.page.parent_id, :conditions => ["virtual = false and status_id = 100 and ? < ?", by, tag.locals.page.send(by)], :order => "#{by} DESC", :limit => 1)
    if previous_page
